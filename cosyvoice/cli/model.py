...
def _make_stream_context(device):
    if device.type == 'cuda':
...
class CosyVoiceModel:
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid):
        ...
        autocast_dtype = self.device.type if self.device.type in ('cuda', 'npu') else 'cpu'
        autocast_enabled = self.fp16 is True and hasattr(self.llm, 'vllm') is False
        with self.llm_context, torch.amp.autocast(device_type=autocast_dtype, enabled=autocast_enabled):
    ...
    def token2wav(self, token, prompt_token, prompt_feat, embedding, uuid, finalize=False, speed=1.0):
        autocast_dtype = self.device.type if self.device.type in ('cuda', 'npu') else 'cpu'
        with torch.amp.autocast(device_type=autocast_dtype, enabled=self.fp16):
        ...
        if finalize is False:
            self.mel_overlap_dict[uuid] = tts_mel[:, :, -self.mel_overlap_len:].detach().clone()
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        with self.lock:
            ...
            self.mel_overlap_dict[this_uuid] = torch.zeros(1, 80, 0, device=self.device)
            self.flow_cache_dict[this_uuid] = torch.zeros(1, 80, 0, 2, device=self.device)
